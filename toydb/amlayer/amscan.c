...
extern int AM_Compare(char *bufPtr, char attrType, int attrLength, char *valPtr);
/* PF function forward declarations (to suppress implicit declaration warnings) */
...
if (AM_scanTable[scanDesc].status != FIRST)
 {
  ...
  { AM_scanTable[scanDesc].status = BUSY;
    memcpy(pageBuf + AM_sl + (AM_scanTable[scanDesc].nextIndex - 1)*recSize,
    AM_scanTable[scanDesc].nextvalue, header->attrLength);
  }

/* copy the recId to be returned */
memcpy(pageBuf + AM_scanTable[scanDesc].nextRecIdPtr,&recId,AM_si);

/* copy the place for next recId */
memcpy(pageBuf + AM_scanTable[scanDesc].nextRecIdPtr + AM_si,
    ...
    if (header->nextLeafPage == AM_NULL_PAGE)
    ...
    else
     {
      ...
      errVal =PF_GetThisPage(AM_scanTable[scanDesc].fileDesc,
        header->nextLeafPage,&pageBuf);
      AM_Check;
      memcpy(pageBuf + AM_sl + header->attrLength,
         &AM_scanTable[scanDesc].nextRecIdPtr,AM_ss);
      errVal = PF_UnfixPage(AM_scanTable[scanDesc].fileDesc
                ,header->nextLeafPage,FALSE);
      AM_Check;
...
static int GetLeftPageNum(fileDesc)
int fileDesc;
...
int pageNum = -1;
int errVal;
AM_INTHEADER inthead,*iheader;
int childPage;
int pageFixed = 0;  /* Track if current page is fixed by us */
...
pageFixed = 1;  /* Page is now fixed by us */
...
/* traverse down the leftmost branch to find the first leaf */
while (*pageBuf != 'l')
  {
    memcpy(iheader,pageBuf,AM_sint);
    memcpy((char *)&childPage,pageBuf + AM_sint,AM_si);
    
    /* Unfix current page before getting next */
    if (pageFixed){
      errVal = PF_UnfixPage(fileDesc,pageNum,FALSE);
      if (errVal != PFE_OK){
        AM_Errno = AME_PF;
    ...
    if (errVal == PFE_PAGEFIXED){
      /* Page already fixed by someone else - don't track it */
      pageFixed = 0;
    } else if (errVal == PFE_OK){
      /* Page fixed by us */
      pageFixed = 1;
    } else {
      /* Error getting page */
