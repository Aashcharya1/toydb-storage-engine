...
static PFReplacementPolicy PFdefaultPolicy = PF_REPL_LRU;
...
static int PFisValidPolicy(policy)
PFReplacementPolicy policy;
{
	return(policy == PF_REPL_LRU || policy == PF_REPL_MRU);
}

static PFReplacementPolicy PFsanitizePolicy(policy)
PFReplacementPolicy policy;
{
	if (!PFisValidPolicy(policy))
		return(PF_REPL_LRU);
	return(policy);
}
...
int i;
	...
	PFhashInit();
	PFStatsInit();
	PFbufSetCapacity(PF_MAX_BUFS);

	/* init the file table to be not used*/
	for (i=0; i < PF_FTAB_SIZE; i++){
		PFftab[i].fname = NULL;
		PFftab[i].policy = PFdefaultPolicy;
	}
}

void PF_SetBufferPoolParams(numbufs)
int numbufs;
{
	if (numbufs <= 0)
		numbufs = 1;
	PFbufSetCapacity(numbufs);
}

void PF_SetDefaultPolicy(policy)
PFReplacementPolicy policy;
{
	PFdefaultPolicy = PFsanitizePolicy(policy);
}

int PF_SetFilePolicy(fd,policy)
int fd;
PFReplacementPolicy policy;
{
	if (PFinvalidFd(fd)){
	...
	PFftab[fd].policy = PFsanitizePolicy(policy);
	return(PFE_OK);
}
...
int fd; /* file descriptor */
	...
	PFftab[fd].policy = PFdefaultPolicy;
...
int PF_OpenFileWithPolicy(fname,policy)
char *fname;
PFReplacementPolicy policy;
{
int fd;
...
PFfpage *fpage;	/* pointer to file page */
	...
	for (temppage= *pagenum+1;temppage<PFftab[fd].hdr.numpages;temppage++){
		if ( (error=PFbufGet(fd,temppage,&fpage,PFreadfcn,
					PFwritefcn,PFftab[fd].policy))!= PFE_OK)
			return(error);
...
PFfpage *fpage;
	...
	if ( (error=PFbufGet(fd,pagenum,&fpage,PFreadfcn,PFwritefcn,
				PFftab[fd].policy))!= PFE_OK){
...
int error;
	...
	if (PFftab[fd].hdr.firstfree != PF_PAGE_LIST_END){
		...
		*pagenum = PFftab[fd].hdr.numpages;
		if ((error=PFbufAlloc(fd,*pagenum,&fpage,PFwritefcn,
					PFftab[fd].policy))!= PFE_OK)
...
void PF_GetStats(stats)
PF_Stats *stats;
{
	PFStatsGet(stats);
}

void PF_PrintStats(out)
FILE *out;
{
	PFStatsPrint(out);
}
