...
extern void PF_SetBufferPoolParams(int numbufs);
extern void PF_SetDefaultPolicy(PFReplacementPolicy policy);
extern int PF_OpenFileWithPolicy(char *fname, PFReplacementPolicy policy);
extern int PF_SetFilePolicy(int fd, PFReplacementPolicy policy);
extern int PF_MarkDirty(int fd, int pagenum);
extern void PF_ResetStats();
extern void PF_GetStats(PF_Stats *stats);
extern void PF_PrintStats(FILE *out);
