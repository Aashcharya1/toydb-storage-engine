...
static int PFmaxBufs = PF_MAX_BUFS;
...
static void PFbufRecordDirty(bpage)
PFbpage *bpage;
{
	if (!bpage->dirty)
		bpage->dirty = TRUE;
	PFStatsIncLogicalWrite();
	PFStatsIncDirtyMark();
}
...
static PFbpage *PFbufSelectVictim(policy)
PFReplacementPolicy policy;
{
PFbpage *tbpage;

	if (policy == PF_REPL_MRU)
...
static int PFbufInternalAlloc(bpage,writefcn,policy)
...
int error;		/* error value returned*/
	...
	if (PFfreebpage != NULL){
		...
		*bpage = NULL;		/* set initial return value */
		...
		tbpage = PFbufSelectVictim(policy);
...
int PFbufGet(fd,pagenum,fpage,readfcn,writefcn,policy)
...
int error;
	...
	PFStatsIncLogicalRead();
	if ((bpage=PFhashFind(fd,pagenum)) == NULL){
		/* page not in buffer. */
		
		/* allocate an empty page */
		if ((error=PFbufInternalAlloc(&bpage,writefcn,policy))!= PFE_OK){
			/* error */
			*fpage = NULL;
			return(error);
		}
...
int PFbufAlloc(fd,pagenum,fpage,writefcn,policy)
int fd;		/* file descriptor */
int pagenum;	/* page number */
